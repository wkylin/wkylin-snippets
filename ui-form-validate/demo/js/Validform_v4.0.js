/*
	...
	$(".demoform").Validform({//$(".demoform")指明是哪一表单需要验证,名称需加在form表单上;
		...
		beforeCheck:function(data){
			//在表单提交执行验证之前执行的函数，data参数是当前表单对象。	
		},
		beforeSubmit:function(data){
			//在表单提交前执行的函数，data参数是当前表单对象。	
		},
		...
		this.each(function(index){
			...
			if(settings.usePlugin){
						...
						custom={
							custom_settings:{
								...
								showmsg:function(msg,type){
									$.fn.Validform.sn.showmsg(msg,settings.tiptype,{obj:swfuploadinput,curform:$this,type:type});	
								}	
							}	
						};
				...
				if(settings.usePlugin.passwordstrength){
					settings.usePlugin.passwordstrength.showmsg=function(obj,msg,type){
						$.fn.Validform.sn.showmsg(msg,settings.tiptype,{obj:obj,curform:$this,type:type,sweep:settings.tipSweep},"hide");
					};
			...
			$this.find("[datatype]").blur(function(){
				...
				if(!flag){
					...
					$.fn.Validform.sn.showmsg($(this).attr("errormsg")||tipmsg.w,settings.tiptype,{obj:$(this),curform:$this,type:3,sweep:settings.tipSweep},"hide"); //当tiptype=1的情况下，传入"hide"则让提示框不弹出,tiptype=2的情况下附加参数"hide"不起作用;
				}else{
					if($(this).attr("ajaxurl")){
						...
						$.fn.Validform.sn.showmsg(tipmsg.c,settings.tiptype,{obj:inputobj,curform:$this,type:1,sweep:settings.tipSweep},"hide");
						$.ajax({
							type: "POST",
							...
							success: function(s){
								if($.trim(s)=="y"){
									...
									$.fn.Validform.sn.showmsg(tipmsg.r,settings.tiptype,{obj:inputobj,curform:$this,type:2,sweep:settings.tipSweep},"hide");
									if(subpost==="postform"){
										$this.trigger("submit");
									}
								}else{
									inputobj.attr("valid",s);
									$.fn.Validform.sn.showmsg(s,settings.tiptype,{obj:inputobj,curform:$this,type:3,sweep:settings.tipSweep});
								}
							},
							error: function(){
								inputobj.attr("valid",tipmsg.err);
								$.fn.Validform.sn.showmsg(tipmsg.err,settings.tiptype,{obj:inputobj,curform:$this,type:3,sweep:settings.tipSweep});	
							}
						});
					}else{
						$.fn.Validform.sn.showmsg(tipmsg.r,settings.tiptype,{obj:$(this),curform:$this,type:2,sweep:settings.tipSweep},"hide");
					}
				}
			...
			var subform=function(){
				...
				$this.find("[datatype]").each(function(){
					...
					if(!inflag){
						...
						$.fn.Validform.sn.showmsg($(this).attr("errormsg")||tipmsg.w,settings.tiptype,{obj:$(this),curform:$this,type:3,sweep:settings.tipSweep});
					...
					if($(this).attr("ajaxurl")){
						if($(this).attr("valid")!="true"){
							var thisobj=$(this);
							$.fn.Validform.sn.showmsg(tipmsg.v,settings.tiptype,{obj:thisobj,curform:$this,type:3,sweep:settings.tipSweep});
				...
				if(flag && !posting){
					...
					if(settings.ajaxPost){
						$.fn.Validform.sn.showmsg(tipmsg.p,settings.tiptype,{obj:$this,curform:$this,type:1,sweep:settings.tipSweep},"alwaysshow");//传入"alwaysshow"则让提示框不管当前tiptye为1还是2都弹出;
						$.ajax({
							type: "POST",
							dataType:"json",
							...
							success: function(data){
								if(data.status==="y"){
									$.fn.Validform.sn.showmsg(data.info,settings.tiptype,{obj:$this,curform:$this,type:2,sweep:settings.tipSweep},"alwaysshow");
								}else{
									posting=false;
									$.fn.Validform.sn.showmsg(data.info,settings.tiptype,{obj:$this,curform:$this,type:3,sweep:settings.tipSweep},"alwaysshow");
								}
							...
							error: function(){
								posting=false;
								$.fn.Validform.sn.showmsg(tipmsg.err,settings.tiptype,{obj:$this,curform:$this,type:3,sweep:settings.tipSweep},"alwaysshow");
							}
						});
						return false;
					}else{
	...
	$.fn.Validform.sn={
		...
		checkform:function(obj,parentobj,tiptype,show){//show用来判断是表达提交还是blur事件引发的检测;
			...
			if(obj.is("[datatype='radio']")){  //判断radio表单元素;
				...
				if(!radiovalue){
					this.showmsg(errormsg,tiptype.type,{obj:obj,curform:parentobj,type:3,sweep:tiptype.sweep},show);
					return false;
				}
				this.showmsg(tipmsg.r,tiptype.type,{obj:obj,curform:parentobj,type:2,sweep:tiptype.sweep},"hide");
				return "radio";
			}
			...
			if(($.trim(obj.val())==="" || obj.val()===defaultvalue) && obj.attr("ignore")!="ignore"){
				this.showmsg(obj.attr("nullmsg") || tipmsg.s,tiptype.type,{obj:obj,curform:parentobj,type:3,sweep:tiptype.sweep},show);
				return false;
			}
